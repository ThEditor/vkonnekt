...
import { units as baseUnits } from "../utils/units";

type TileStatus = "LOCKED" | "ACTIVE" | "COMPLETE";

const tileStatus = (tile: Tile, lessonsCompleted: number, units: Unit[]): TileStatus => {
      ...
      >
        ...
        {status === "ACTIVE" ? (
          <Link
            href="/lesson"
            onClick={() => {
              localStorage.setItem("unit", unit?.description ?? "");
              localStorage.setItem("tile", unit?.tiles[index].description ?? "");
            }}
            className={[
...
const UnitSection = ({ unit, units }: { unit: Unit; units: Unit[]; }): JSX.Element => {
    ...
    <>
      ...
      <div className="relative mb-8 mt-[67px] flex max-w-2xl flex-col items-center gap-4">
        {unit.tiles.map((tile, i): JSX.Element => {
          const status = tileStatus(tile, lessonsCompleted, units);
          return (
            <Fragment key={i}>
              {(() => {
                switch (tile.type) {
...
const buildUnits = async (language: string) => {
  // query api/units & return units
  const response = await fetch(`https://positive-clearly-tiger.ngrok-free.app/courses/${language}`, {
    method: 'GET',
    headers: new Headers({
  ...
  return modules.map((module: any, i: number) => ({
    ...
    borderColor: `border-[#7a59e6]`,
    tiles: module.subheadings.map((subheading: string, i: number) => ({
      type: i % 2 == 0 ? 'star' : 'book',
...
const Learn: NextPage = () => {
  ...
  useEffect(() => {
    (async () => setUnits(await buildUnits(language.name)))();
  }, []);
  ...
  const topBarColors = getTopBarColors(scrollY, units);
    ...
    <>
      ...
      <div className="flex justify-center gap-3 pt-14 sm:p-6 sm:pt-10 md:ml-24 lg:ml-64 lg:gap-12">
        <div className="flex max-w-2xl grow flex-col">
          {units.map((unit) => (
            <UnitSection unit={unit} key={unit.unitNumber} units={units} />
          ))}
...
const UnitHeader = ({
  ...
  return (
    <article
      className={["max-w-2xl text-white sm:rounded-xl bg-[#1b2245]"].join(
        " ",
      )}
    >
      <header className="flex items-center justify-between gap-4 p-4">
        ...
        {/* <Link
          href={`https://vKonnekt.com/guidebook/${language.code}/${unitNumber}`}
          className={[
