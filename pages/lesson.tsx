...
const formatTime = (timeMs: number): string => {
  ...
  if (hours === 0) return [minutes, seconds].map((x) => x.toString().padStart(2, '0')).join(':');
...
const Lesson: NextPage = () => {
  ...
  const [lessonProblems, setLessonProblems] = useState<typeof lessonProblem1[]>([]);
  const language = useBoundStore((x) => x.language);

  useEffect(() => {
    const unit = localStorage.getItem('unit') ?? 'Module 1: Python Basics Introduction to Python';
    const tile = localStorage.getItem('tile') ?? 'What is Python?';
    (async () => {
      const response = await fetch(`https://positive-clearly-tiger.ngrok-free.app/questions/${language}/${unit}/${tile}`, {
        headers: new Headers({
        "ngrok-skip-browser-warning": "69420",
      }),
      });
      const lessonProblems = await response.json();
      console.log(lessonProblems);
      setLessonProblems(
        lessonProblems.questions.map((q: any) => {
          return {
            type: 'SELECT_1_OF_3',
            question: q.question,
            answers: [
  ...
  const totalCorrectAnswersNeeded = lessonProblems.length / 2;
  ...
  if (hearts !== null && isStartingLesson) {
    return (
      <LessonFastForwardStart unitNumber={unitNumber} setIsStartingLesson={setIsStartingLesson} />
    );
  }
  ...
  return (
    <header className="flex items-center gap-4">
      {correctAnswerCount === 0 ? (
        <Link href="/learn" className="text-gray-300">
          <CloseSvg />
          <span className="sr-only">Exit lesson</span>
        </Link>
      ) : (
        <button className="text-gray-300" onClick={() => setQuitMessageShown(true)}>
          <CloseSvg />
          <span className="sr-only">Exit lesson</span>
        </button>
      )}
    ...
    <>
      ...
      >
        <div className="flex grow flex-col gap-4">
          <h2 className="text-lg font-bold sm:text-2xl">Are you sure you want to quit?</h2>
          <p className="text-gray-500 sm:text-lg">All progress for this lesson will be lost.</p>
        </div>
        <div className="flex grow flex-col items-center justify-center gap-4 sm:flex-row-reverse">
          ...
          <button
            className="w-full rounded-2xl py-3 font-bold uppercase text-blue-400 transition hover:brightness-90 sm:w-48 sm:border-2 sm:border-b-4 sm:border-gray-300 sm:text-gray-300 sm:hover:bg-gray-100"
            onClick={() => setQuitMessageShown(false)}
          >
            Stay
          </button>
    ...
    <>
      <section className="border-gray-200 sm:border-t-2 sm:p-10">
        <div className="mx-auto flex max-w-5xl sm:justify-between">
          <button
            className="hidden rounded-2xl border-2 border-b-4 border-gray-200 bg-white p-3 font-bold uppercase text-gray-300 transition hover:border-gray-300 hover:bg-gray-200 sm:block sm:min-w-[150px] sm:max-w-fit"
            onClick={onSkip}
          >
            Skip
          </button>
          {!isAnswerSelected ? (
            <button
              className="grow rounded-2xl bg-gray-200 p-3 font-bold uppercase text-gray-300 sm:min-w-[150px] sm:max-w-fit sm:grow-0"
              disabled
            >
              Check
            </button>
    ...
    <div className="flex min-h-screen flex-col gap-5 px-4 py-5 sm:px-0 sm:py-0">
      <div className="flex grow flex-col items-center gap-5">
        ...
        <section className="flex max-w-2xl grow flex-col gap-5 self-center sm:items-center sm:justify-center sm:gap-24 sm:px-5">
          ...
          <div className="grid grid-rows-4 gap-2 sm:grid-rows-4" role="radiogroup">
      ...
      <QuitMessage quitMessageShown={quitMessageShown} setQuitMessageShown={setQuitMessageShown} />
    </div>
  );
};
        ...
        <div className="flex flex-wrap justify-center gap-5">
          ...
          <div className="min-w-[110px] rounded-xl border-2 border-green-400 bg-green-400">
            ...
            <div className="flex justify-center rounded-xl bg-white py-4 text-green-400">
              {Math.round((correctAnswerCount / (correctAnswerCount + incorrectAnswerCount)) * 100)}
              %
            </div>
          </div>
        </div>
  ...
  return (
    ...
    >
      ...
      <div className="relative flex w-full max-w-4xl flex-col gap-5 rounded-2xl border-2 border-gray-200 bg-white p-8">
        <button
          className="absolute -right-5 -top-5 rounded-full border-2 border-gray-200 bg-gray-100 p-1 text-gray-300 hover:brightness-90"
          onClick={() => setReviewLessonShown(false)}
        >
              ...
              <button
                ...
                onClick={() =>
                  setSelectedQuestionResult((selectedQuestionResult) =>
                    selectedQuestionResult === questionResult ? null : questionResult,
                  )
                }
              >
                ...
                {selectedQuestionResult === questionResult && (
                  <div className="absolute left-1 right-1 top-20 z-10 rounded-2xl border-2 border-gray-200 bg-white p-3 text-sm tracking-tighter">
                    ...
                    ></div>
                    <div className="font-bold uppercase text-gray-300">Your response:</div>
                    <div className="mb-3 text-gray-700">{questionResult.yourResponse}</div>
                    <div className="font-bold uppercase text-gray-300">Correct response:</div>
                    <div className="text-gray-700">{questionResult.correctResponse}</div>
                  </div>
                )}
              </button>
            );
  ...
  return (
    <div className="flex min-h-screen flex-col px-5 py-8 text-center">
      <div className="flex grow flex-col items-center justify-center gap-5">
        <LessonFastForwardStartSvg />
        <h1 className="text-lg font-bold">Want to jump to Unit {unitNumber}?</h1>
...
const LessonFastForwardEndFail = ({
  ...
  return (
    <div className="flex min-h-screen flex-col px-5 py-8 text-center">
      <div className="flex grow flex-col items-center justify-center gap-5">
        <LessonFastForwardEndFailSvg />
        <h1 className="text-2xl font-bold">{`You didn't unlock Unit ${unitNumber}`}</h1>
        <p className="text-lg text-gray-500">{`Don't worry! Practice makes perfect.`}</p>
      </div>
      <section className="border-gray-200 sm:border-t-2 sm:p-10">
        <div className="mx-auto flex max-w-5xl sm:justify-between">
          <button
  ...
  return (
    <div className="flex min-h-screen flex-col px-5 py-8 text-center">
      <div className="flex grow flex-col items-center justify-center gap-5">
        ...
        <p className="text-lg text-gray-500">Way to go! You’re making great strides!</p>
      </div>
      <section className="border-gray-200 sm:border-t-2 sm:p-10">
        <div className="mx-auto flex max-w-5xl sm:justify-between">
          <button
